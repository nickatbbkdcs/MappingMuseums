...
# - - - - - -  - - - - - -  - - - - - -  - - - - - -  - - - - - -  - - - - - -  - - - - - - 
# NOT TESTED PROPERLY
    def add_nodewithprefix(self, node, parent=None,prefix=None):
        """
        ...
        if pid is None:
            if self.root is not None:
                ...
                self.root = prefix+"/"+node.identifier
        elif not self.contains(pid):
            raise NodeIDAbsentError("Parent node '%s' "
                                    "is not in the tree" % pid)

        self._nodes.update({prefix+"/"+node.identifier: node})
        self.__update_fpointer(pid, prefix+"/"+node.identifier, Node.ADD)
        self.__update_bpointer(prefix+"/"+node.identifier, pid)
...
# - - - - - -  - - - - - -  - - - - - -  - - - - - -  - - - - - -  - - - - - -  - - - - - - 
	...
	for kid in children:
	    count=count + self.countChildren(kid)
		
	return count

    def hasChildWithName(self,node,name):
	children=self.children(node.identifier)
	for kid in children:
	    if (kid.tag == name):
		return kid
    ...
    def getPathForNodeIds(self,node):
        res = []

	res.append([nid for nid in self.rsearch(node.identifier)][::-1])

        return res

    def getPathForNode(self,node):
        res = []
	for nid in self.rsearch(node.identifier):
	    #res.append(nid)
	    res.append(self[nid].tag)
	return res[::-1]
